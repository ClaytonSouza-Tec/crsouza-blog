...
function normalizeBaseUrl(urlValue) {
  return String(urlValue || "").trim().replace(/\/index\.html$/i, "").replace(/\/$/, "");
}

function isPlaceholderUrl(urlValue) {
  return /seu-dominio\.com|example\.com/i.test(String(urlValue || ""));
}

function getPublicBaseUrl() {
  const blogUrl = normalizeBaseUrl(process.env.BLOG_URL);
  const websiteHostname = String(process.env.WEBSITE_HOSTNAME || "").trim();

  if (blogUrl && !isPlaceholderUrl(blogUrl)) {
    return blogUrl;
  }

  if (websiteHostname) {
    return normalizeBaseUrl(`https://${websiteHostname}`);
  }
...
function buildNewsEmailText(name, newsItems, email) {
  ...
  const body = newsItems
    ...
    .join("\n\n---\n\n");
  const cta = "\nAcesse o blog e contribua com o seu comentario sobre o artigo.";
...
function buildNewsEmailHtmlWithFooter(name, newsItems, email) {
  const articles = newsItems
    .map((item) => {
      const articleUrl = buildNewsItemUrl(item.url);
      const imageUrl = item.image ? buildNewsItemUrl(item.image) : "";
      const imageHtml = imageUrl ? `<img src="${imageUrl}" alt="${item.imageAlt || item.title}" style="width: 100%; height: auto; display: block; margin: 0 0 16px; border-radius: 8px;" />` : "";
      ...
      <tr>
        <td style="padding: 0 0 24px;">
          ...
          <h2 style="margin: 0 0 12px; font-size: 20px; color: #111827; line-height: 1.3;">${item.title}</h2>
          <p style="margin: 0 0 12px; color: #374151; line-height: 1.6; font-size: 14px;">${item.summary}</p>
          <p style="margin: 16px 0 0;"><a href="${articleUrl}" style="display: inline-block; color: #ffffff; background: #2563eb; padding: 10px 20px; text-decoration: none; border-radius: 6px; font-weight: 600;">Ler artigo completo</a></p>
        </td>
      </tr>
    `;
    })
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto;">
      ...
      <tr>
        <td style="padding: 24px; background: #ffffff;">
          ...
          <p style="margin: 0 0 16px; color: #111827; font-weight: 600; text-align: center;">Acesse o blog e contribua com o seu comentario sobre o artigo.</p>
